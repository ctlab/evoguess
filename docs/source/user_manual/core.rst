...
Главный пакет библиотеки EvoGuess. В ядре описывается логика взаимодействия всех зависимых модулей для выполнения требуемой задачи. Основная задача, которую позволяется решить библиотека EvoGuess, является задача метаэвристической оптимизации целевой black-box function.
